...
% backed up _2013_12_12 before uncommenting last lines for var_scaled. gh
...
for iter = 1:options.maxIter
    ...
    %jj - Gamma added 6/6/12%
    [wa wpi xbar S Nk lnZ(iter) Gamma] = forwbackFRET(astar,pXgivenZtilde,pistar,x);  
    ...
    if (iter>2)  % Murphy choice = 2; change to 30 on 2013_05_23 gholz  
...
out.v = v;
out.F = F;
%jj - Nk and Gamma added 6/6/12
out.Nk= Nk;
out.Gamma = Gamma;
% lines below were commented out until 2013_12_12.  gh
out.scaledVar = zeros(D,D,K);
% mode variance of the data analyzed (which might have been scaled)
for k=1:K
    out.scaledVar(:,:,k)=((inv(out.W(:,:,k))-inv(PriorPar.W))/(out.v(k)-D-1));
end