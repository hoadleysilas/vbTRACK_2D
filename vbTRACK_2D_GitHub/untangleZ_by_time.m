...
% with Kfav states, and sorts them. The sorting criterion is time  
...
fprintf('Step 5.2 Start untangleZ_by_time\n');


% --- Step 1 find the index of data points associated with state=kk for model=Kfav
% using current z_hat
indexofZ = cell(1,Kfav);  % cols designate kk
mean_indexofZ = zeros(1,Kfav); % criterion for untangling 2D code
...
model = Kfav;     % loop over models
    for kk = 1:Kfav % loop over states within a model
        ...
        mean_indexofZ(1,kk)=mean(indexofZ{kk,Kfav});
        z_hat_value(1,kk) = kk;
    end
%     
...
untangleMatrix(:,6) = mean_indexofZ(1,:)';                      % added 2019_01_07 gh
    
    % if (VT == 'V')
        untangleMatrix_sort = sortrows(untangleMatrix,6);
       %  untangleMatrix_sort_flip = flipud(untangleMatrix_sort);
        untangleMatrix_sort_flip = untangleMatrix_sort; % NO FLIP
